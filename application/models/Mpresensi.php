...
class Mpresensi extends CI_ModeL {
	...
	public function load_presensi(){
		$sql = $this->db->query("SELECT terlambat.tanggal_terlambat, terlambat.keterlambatan, pegawai.email, kategori.nama_kategori, terlambat.keterangan FROM ((terlambat INNER JOIN pegawai ON terlambat.id_pegawai = pegawai.id_pegawai) INNER JOIN kategori ON terlambat.id_kategori = kategori.id_kategori)");
